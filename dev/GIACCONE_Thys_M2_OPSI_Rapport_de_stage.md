...
L'eye-tracker est positionné sous l'écran de jeu et est calibré à l'aide du logiciel natif *EyeTribe UI*. TheEyeTribe, considéré comme un système d'eye-tracking low-cost, ne présente pas pour autant de mauvaises performances, comme le confirme le papier de *Kristien Ooms et al.* (2015), tant que l'on impose de conditions expérimentales contrôlées : le sujet doit rester relativement immobile à une distance fixe du dispositif.
...
L'étape de calibration consiste en la fixation et au suivi, par le sujet, d'un point qui apparait à l'écran puis se déplace en 16 endroits différents, marquant un arrêt d'environ une seconde à chaque emplacement. Si le sujet a correctement suivi le point des yeux, le logiciel confirme la qualité de la calibration et l'étape est terminée. Dans le cas contraire, le logiciel indique une mauvaise calibration et le sujet la recommence alors.
...
Figure 5 : *Comportement du regard (dégradé bleu-rouge) selon l'axe Y et fonction du temps lors du rebond de la balle (noir) sur une brique*  
...
- **Niveau 1 (p = 0) :** On observe une faible quantité de données pour le niveau 1, ce qui rend l'interprétation des résultats difficile. On peut néanmoins comprendre ce manque après réflexion ; les briques étant entièrement prédictibles pour le sujet, car se comportant comme lors d'un rebond réel et du fait de leur probabilité **p = 0**, le joueur peut alors se permettre de poser son regard ailleurs et utiliser sa vision périphérique pour traquer la balle.  
